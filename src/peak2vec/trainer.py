...
from src.peak2vec.dataset import PeakDataset, peak2vec_collate
from src.peak2vec.models import Peak2VecModel
from src.peak2vec.preprocess import prepare_adata, get_sampling_distributions, balanced_downsample
from src.peak2vec.config import ExperimentConfig
from src.peak2vec.utils.device import resolve_device
from src.peak2vec.utils.io import ensure_dir, save_json, save_npy, save_yaml
from src.peak2vec.utils.seed import seed_everything
...
def _init_wandb(cfg: ExperimentConfig) -> Optional[wandb.sdk.wandb_run.Run]:
    ...
    if cfg.wandb.mode == "disabled":
        return None
    if wandb is None:
        raise RuntimeError("wandb is not installed but wandb.mode != 'disabled'. Install wandb or disable it.")
...
def train(cfg: ExperimentConfig, *, verbose: bool = False) -> None:
    ...
    adata = prepare_adata(
        ...
        peak_name_source=cfg.preprocessing.peak_name_source,
        overwrite_coords=cfg.preprocessing.overwrite_coords,
    )
    ...
    if cfg.wandb.mode != "disabled" and cfg.wandb.save_table:
        ...
        downsampled_peaks = balanced_downsample(adata.var[cfg.preprocessing.chrom_col].to_frame().reset_index(), cfg.preprocessing.chrom_col, cfg.preprocessing.n_per_chromosome)
    ...
    for epoch in track(range(1, cfg.train.epochs + 1), description="Training epochs"):
        ...
        if epoch % cfg.train.checkpoint_every_epochs == 0:
            ...
            if run is not None:
                artifact = wandb.Artifact(f"{run.name}_checkpoint_epoch{epoch:04d}", type="model")
                artifact.add_file(str(checkpoint_path))
                run.log_artifact(artifact)
        
        # Save table of embeddings to W&B
        if epoch % cfg.train.save_embeddings_every_epochs == 0 and cfg.preprocessing.save_table and run is not None:
            embedding_norm = model.get_peak_embeddings(normalize=True).numpy()
            emb_df = pd.DataFrame(embedding_norm[downsampled_idx], columns=[f"dim_{i}" for i in range(embedding_norm.shape[1])])
